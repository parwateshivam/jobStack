...
async function createJob(req, res) {
  try {
    ...
    if (!company) {
      throw ("Invalid request. Please register/login first !")
    }

    let { title, jobRequirements } = req.body

    if (!title || !jobRequirements) {
      throw ("invalid or missing data to create job !")
    }
    ...
    let JobData = await newJob.save()

    let updateCompany = await companyModel.findByIdAndUpdate(
      company._id, { $push: { "createJobs": JobData._id } }
    )

    if (updateCompany.modifiedCount == 0) {
      throw ("unable to store job in company data !")
    }
...
async function handleJobApplication(req, res) {
  try {
    ...
    let updateJob = await jobModel.findByIdAndUpdate(
      jobId, { $push: { "applications": user._id } }
    )

    let updateUser = await userModel.findByIdAndUpdate(
      user._id, { $push: { "appliedJobs": jobId } }
    )

    if (updateJob.modifiedCount == 0)
      throw ("unable to apply for a job !")
